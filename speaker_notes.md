...
> The first category. Social bias is perhaps the most studied — training data reflects societal biases and models amplify them. This has been measured extensively using benchmarks like WinoBias and StereoSet. For example, occupational gender bias is pervasive: when given a gender-neutral sentence about a doctor and a nurse, LLMs overwhelmingly default to assuming the doctor is male and the nurse is female. Toxicity is counterintuitive — it actually increases with model size. Larger models have more capacity to produce toxic outputs, even from benign prompts. On the right — hallucinations are split into two types. Factuality hallucinations contradict established facts. Faithfulness hallucinations deviate from context the user actually provided. The key takeaway at the bottom: these risks aren't confined to one stage — they emerge across the entire model lifecycle, from data collection through deployment.
