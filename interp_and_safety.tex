...
\begin{frame}[plain]
  ...
  \begin{columns}[T]
    ...
    \begin{ucrinfo}
      ...
      My teammates covered \textbf{what} interpretability is, and \textbf{what} safety risks and guardrails exist. I connect the two: how interpretability methods \emph{actually} helps us build safer LLMs.
    \end{ucrinfo}
  \end{columns}
\end{frame}
...
%  OVERVIEW FIGURE
% ═══════════════════════════════════════════════════════════════════
\begin{frame}{Interpretability for Safety: Overview}
    \centering
    \includegraphics[width=0.95\textwidth,height=0.82\textheight,keepaspectratio]
    {Interpretability_for_Safety_Flow_Chart.png}
\end{frame}

\begin{frame}{\S3.1\ Training Data Attribution (TDA)}

{\scriptsize\itshape
Core hypothesis: unsafe outputs stem from problematic training data.
...
\scriptsize
Compare similarity of training-example latent vectors to the output.
Fast, but does not establish causality.
\end{ucrcard}

\vspace{1pt}

\begin{ucrcard}[UCRnavy]{Gradient-Based}
\scriptsize
Influence Functions and TracIn estimate how removing a training example
would affect model behavior.
\end{ucrcard}
...
\scriptsize
Measures each training example's contribution to model performance.
Principled but computationally expensive.
\end{ucrcard}

\column{0.53\textwidth}

{\footnotesize\bfseries\navy{Example Attribution Workflow}}
...
{\scriptsize\itshape
Once influential training examples are identified, they can be analyzed,
filtered, or removed to improve model behavior.
}
...
\item \textbf{Hallucination Sources}
      LLM Attributor traces generated false facts back to specific
      training examples.

\item \textbf{Bias Origins}
      Identifies documents introducing gender, racial, or cultural
      stereotypes.

\item \textbf{Toxic Outputs}
      Links harmful generations to problematic training passages.

\item \textbf{Training Dynamics}
      Tracks how capabilities such as refusal behavior emerge during
      training.
\end{itemize}
...
\scriptsize
Most frontier models use proprietary datasets.
Using TDA to improve safety through data removal and retraining has only
been demonstrated on relatively small models. Scaling remains unsolved due to the sheer magnitude of training data and the immense compute for each of these methods.
\end{ucrwarn}
...
\begin{frame}{\S3.3.2\ Perturbing Components to Find Safety Mechanisms}
  ...
  \begin{columns}[T]
    ...
    \begin{itemize}\scriptsize\setlength\itemsep{4pt}
      ...
      \textbf{But what about polysemanticity?}
      \item \textbf{Activation Patching}: replace intermediate activations from one input with another input's activations, eg. safe vs unsafe flagged inputs, directly measures the causal effect of a specific region in terms of knowledge storage or biases induced.
...
Most neurons are \textbf{polysemantic} --- they encode multiple unrelated concepts simultaneously.
SAEs disentangle them into interpretable features.
}
...
\item Encoder maps a polysemantic activation
      $\rightarrow$ sparse concept vector

\item Each dimension (SAE neuron)
      represents a distinct concept

\item Decoder reconstructs the original activation

\item Highly activating inputs reveal
...
\begin{ucrdark}[UCRnavy]
\scriptsize\bfseries Anthropic's Scaling Monosemanticity (2024)\\[2pt]

\scriptsize
Applied to Claude 3 Sonnet, SAEs uncovered millions of
interpretable features including deception, bias,
sycophancy, dangerous content, and the famous
\emph{Golden Gate Bridge} feature.
\end{ucrdark}

\column{0.53\textwidth}

{\small\bfseries\navy{Polysemantic $\rightarrow$ Monosemantic}}
...
\begin{frame}{\S3.3.3\ SAEs for Safety Intervention}

{\footnotesize\itshape
Once interpretable features are identified, they can be manipulated directly
to improve safety.
}
...
\scriptsize
\textbf{Härle et al. (2024):}
SCAR identifies SAE features activated during jailbreak attempts.
Suppressing them blocks attacks without retraining.
\end{ucrcard}
...
\scriptsize
\textbf{Hegde (2024)} and \textbf{Zhou et al. (2025)}
identify SAE features encoding gender bias.
Clamping them reduces biased outputs.
\end{ucrcard}
...
\scriptsize
\textbf{Frikha et al. (2025):}
PrivacyScalpel locates neurons encoding memorized personal information,
enabling targeted privacy protection.
\end{ucrcard}
...
\scriptsize
Projects intermediate hidden states onto the vocabulary,
revealing what the model is ``thinking'' at each layer and
helping detect hallucinations before generation completes.
\end{ucrinfo}
...
\begin{ucrwarn}
\scriptsize\bfseries Critical Caveat: Unfaithful Explanations \\
\normalfont\scriptsize
CoT explanations often do not reflect the model's true internal computation.
Self-reasoning is therefore a useful signal, not a guarantee of safety.
\end{ucrwarn}
...
\begin{frame}{\S4.1 \& \S4.2.1\ Token-Level Fixes and Representation Steering}
  ...
  \begin{columns}[T]
    ...
    \begin{itemize}\scriptsize
      ...
      \item Remove jailbreak-triggering tokens identified by attribution
      (\textit{Pan et al., 2025})

      \item Amplify attention to reliable user-specified tokens
      (\textit{Zhang et al., 2024})

      \item Redirect model focus to relevant context to reduce RAG hallucinations
      (\textit{Liu et al., 2025})
    ...
    \begin{ucrcard}[UCRnavy]{Steering Vectors}
      ...
      Add the ``safe direction'' discovered through probing
      directly to the hidden state:
      ...
      h
      +
      \alpha \hat{v}_{\text{safe}}
      \]

      \scriptsize

      \textbf{No retraining required.}
      Works entirely at inference time and can be tuned for different tasks.
...
% ═══════════════════════════════════════════════════════════════════
%  SLIDE 12 --- §4.2 + §4.3 VERIFY & SELF-REASONING OUTPUT
% ═══════════════════════════════════════════════════════════════════
\begin{frame}{\S4.2 \& \S4.3\ Verify Before Output + Self-Reasoning Safety}
  \begin{columns}[T]
    \column{0.48\textwidth}
    {\small\bfseries\navy{\S4.2 --- Verify Before Output}}
    ...
    \column{0.48\textwidth}
    {\small\bfseries\navy{\S4.3 --- Output with Self-Reasoning}}
...
\begin{frame}{Conclusion: Interpretation is the Path to Principled Safety}
  ...
  \begin{columns}[T]
    ...
    \begin{ucrdark}[UCRnavy]
      ...
      Interpretability gives us the tools to \textbf{find} those causes and \textbf{fix them precisely}.
    \end{ucrdark}
    \vspace{0.4em}
    {\small\bfseries\navy{Key Takeaways}}
    \begin{itemize}\scriptsize\setlength\itemsep{3pt}
      ...
      \item Interpretability and Safety research needs to go hand-in-hand
    \end{itemize}
  \end{columns}
\end{frame}
